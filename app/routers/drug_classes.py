...
from app.services.drug_classes import get_drug_classes
...
router = APIRouter(tags=["drug_classes"])
logger = structlog.get_logger(__name__)


@router.get("/drug/{drug_id_1mg}/drug-classes")
async def drug_classes(drug_id_1mg: str, request: Request):
    ...
    cache_key = build_key("drug_classes", resolved.formulation_id)
    ...
    if cached_hit:
        ...
        data = await get_drug_classes(resolved.formulation_id, pool)
